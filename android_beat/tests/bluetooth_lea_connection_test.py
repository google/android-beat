...
"""Bluetooth LE Audio connection test."""
...
_BLUETOOTH_UNPAIRING_TIMEOUT = datetime.timedelta(seconds=60)


class BluetoothLeaConnectionTest(base_test.BaseTestClass):
  """Connection tests for Bluetooth LE Audio."""
  ...
  def test_pairing(self):
    ...
    # Verify LE Audio Bluetooth profile are successfully linked and Android
  ...
  def test_unpairing(self):
    ...
    Objective:
      To validate the Device Under Test (DUT) can successfully unpair with a
      paired Bluetooth device (BT device).
    ...
    Test Steps:
      ...
      2. DUT initiates Bluetooth discovery.
      3. DUT pairs with the BT device.
      4. DUT unpairs with the BT device.

    Pass Criteria:
      1. DUT should disconnect A2DP/HFP (Classic) or LE Audio connection.
      2. The saved device in DUT should not include the BT device.
    """
    # BT device starts pairing mode.
    bluetooth_utils.start_pairing_mode(
        self.bt_device, timeout=_BLUETOOTH_DISCOVERY_TIMEOUT
    ...
    # Verify LE Audio profile are successfully linked and Android system
    ...
    # DUT unpairs with the BT device and verify the unpairing is successful.
    self.ad.bt_snippet.btUnpairDevice(self.bt_device.bluetooth_address_primary)

    # Verify LE Audio profile are successfully unlinked and Android system
    # recognizes the correct audio device type.
    bluetooth_utils.wait_and_assert_lea_state(
    ...
    test_utils.wait_until_or_assert(
        condition=lambda: not bluetooth_utils.is_bt_device_in_saved_devices(
            self.ad, self.bt_device.bluetooth_address_primary
        ),
        error_msg='Failed to unpair with Bluetooth device',
        timeout=_BLUETOOTH_UNPAIRING_TIMEOUT,
    )
